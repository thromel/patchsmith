...
- Source size: 267 Python files under `src/patchsmith`, about 59.4k lines.
- Test size: 93 Python files under `tests`, about 32.9k lines.
...
| --- | ---: | ---: | --- |
| root modules | 80 | 18122 | Agent shell compatibility, DeepAgents, workflow, retrieval, patching, models, safety, and mixed helpers are still colocated. |
...
| chat | 23 | 4115 | Command handlers, command registry, task execution, resume hydration, controller lifecycle, and shared replay helpers are split out. |
...
| --- | ---: | --- |
| `chat/controller.py` | 9 | Chat controller owns REPL/session glue, command dispatch, custom commands, hooks, and workflow callbacks while registry assembly is isolated. |
