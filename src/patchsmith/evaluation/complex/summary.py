...
def complex_summary(
    *,
    ...
    attempted = [
        result for result in results if result.status in {"validated", "failed"}
    ]
    providers = sorted(
        {
            result.model_provider
            for result in attempted
            if result.model_provider is not None
        }
    )
    ...
    attempted_by_task = {
        ...
        if any(result.status in {"validated", "failed"} for result in task_results)
    }
    tasks_with_validated_attempt = sum(
        1
        for task_results in attempted_by_task.values()
        if any(result.validation_passed for result in task_results)
    )
    tasks_with_failed_attempts_only = sum(
        1
        for task_results in attempted_by_task.values()
        if not any(result.validation_passed for result in task_results)
    )
    ...
    attempted_with_target_alignment = [
        result for result in attempted if result.patch_target_aligned is not None
    ]
    target_aligned_tasks = sum(
        1 for result in attempted_with_target_alignment if result.patch_target_aligned
    )
    (
    ...
    return ComplexBenchmarkSummary(
        ...
        reproduced_tasks=sum(1 for result in results if result.reproduced),
        validated_tasks=sum(1 for result in results if result.validation_passed),
        failed_tasks=sum(1 for result in attempted if not result.validation_passed),
        blocked_tasks=sum(1 for result in results if result.status == "blocked"),
        preflight_passed_tasks=sum(
            1 for result in results if result.preflight_status == "passed"
        ),
        preflight_skipped_tasks=sum(
            1 for result in results if result.preflight_status == "skipped"
        ),
        preflight_blocked_tasks=sum(
            1 for result in results if result.preflight_status == "blocked"
        ),
        ...
        patch_generated_rate=_rate(
            sum(1 for result in attempted if result.patch_generated),
            len(attempted),
        ),
        validation_rate=_rate(
            sum(1 for result in attempted if result.validation_passed),
            len(attempted),
        ),
        reproduced_input_rate=_rate(
            sum(1 for result in results if result.reproduced),
            len(results),
        ),
        avg_trace_events=_average(result.trace_event_count for result in attempted),
        avg_runtime_nodes=_average(result.runtime_node_count for result in attempted),
        failed_trace_event_count=sum(
            result.failed_trace_event_count for result in attempted
        ),
        avg_retry_events=_average(result.retry_event_count for result in attempted),
        retry_feedback_artifact_tasks=sum(
            1 for result in attempted if result.retry_feedback_artifact_count > 0
        ),
        retry_feedback_artifact_count=sum(
            result.retry_feedback_artifact_count for result in attempted
        ),
        retry_label_counts=_merge_label_counts(
            result.retry_label_counts for result in attempted
        ),
        retry_failure_class_counts=_merge_label_counts(
            result.retry_failure_class_counts for result in attempted
        ),
        avg_deepagents_virtual_file_count=_average_optional(
            result.deepagents_virtual_file_count for result in attempted
        ),
        context_budgeted_tasks=sum(
            1 for result in attempted if result.deepagents_context_budgeted
        ),
        context_budget_manifest_tasks=sum(
            1
            for result in attempted
            if result.deepagents_context_budget_manifest_path is not None
        ),
        context_budget_omitted_file_count=sum(
            result.deepagents_context_budget_omitted_file_count or 0
            for result in attempted
        ),
        avg_context_budget_omitted_files=_average_optional(
            result.deepagents_context_budget_omitted_file_count
            for result in attempted
            if result.deepagents_context_budget_manifest_path is not None
        ),
        repo_map_manifest_tasks=sum(
            1
            for result in attempted
            if result.deepagents_repo_map_manifest_path is not None
        ),
        repo_instructions_manifest_tasks=sum(
            1
            for result in attempted
            if result.deepagents_repo_instructions_manifest_path is not None
        ),
        repo_instructions_read_first_rate=_average(
            (
                1.0
                if result.deepagents_repo_instructions_manifest_read_first
                else 0.0
            )
            for result in attempted
            if result.deepagents_repo_instructions_manifest_path is not None
        ...
        acceptance_rubric_manifest_tasks=sum(
            1
            for result in attempted
            if result.deepagents_acceptance_rubric_manifest_path is not None
        ),
        acceptance_rubric_read_first_rate=_average(
            (
                1.0
                if result.deepagents_acceptance_rubric_manifest_read_first
                else 0.0
            )
            for result in attempted
            if result.deepagents_acceptance_rubric_manifest_path is not None
        ...
        acceptance_rubric_aligned_tasks=sum(
            1
            for result in attempted
            if result.deepagents_acceptance_rubric_aligned is True
        ),
        acceptance_rubric_alignment_rate=_average(
            1.0 if result.deepagents_acceptance_rubric_aligned is True else 0.0
            for result in attempted
            if result.deepagents_acceptance_rubric_manifest_path is not None
        ),
        repair_interface_manifest_tasks=sum(
            1
            for result in attempted
            if result.deepagents_repair_interface_manifest_path is not None
        ),
        repair_interface_read_first_rate=_average(
            1.0 if result.deepagents_repair_interface_manifest_read_first else 0.0
            for result in attempted
            if result.deepagents_repair_interface_manifest_path is not None
        ),
        avg_deepagents_max_context_files=_average_optional(
            result.deepagents_max_context_files
            for result in attempted
            if result.deepagents_context_budgeted
        ),
        resource_budgeted_tasks=sum(
            1 for result in attempted if result.deepagents_resource_budgeted
        ),
        resource_budget_read_first_rate=_average(
            1.0 if result.deepagents_resource_budget_read_first else 0.0
            for result in attempted
            if result.deepagents_resource_budgeted
        ),
        avg_resource_budget_max_model_responses=_average_optional(
            result.deepagents_resource_budget_max_model_responses
            for result in attempted
            if result.deepagents_resource_budgeted
        ),
        avg_resource_budget_max_model_tokens=_average_optional(
            result.deepagents_resource_budget_max_model_tokens
            for result in attempted
            if result.deepagents_resource_budgeted
        ),
        quality_warning_tasks=sum(
            1 for result in attempted if result.patch_quality_warning
        ),
        quality_warning_rate=_rate(
            sum(1 for result in attempted if result.patch_quality_warning),
            len(attempted),
        ),
        ...
        avg_debuggability_score=_average(
            result.debuggability_score for result in attempted
        ),
        avg_agent_trajectory_score=_average(
            result.agent_trajectory_score for result in attempted
        ),
        todo_planning_rate=_average(
            1.0 if result.todo_planning else 0.0 for result in attempted
        ),
        constrained_filesystem_rate=_average(
            1.0 if result.constrained_filesystem else 0.0 for result in attempted
        ),
        specialist_review_rate=_average(
            1.0 if result.specialist_review else 0.0 for result in attempted
        ),
        guardrails_rate=_average(
            1.0 if result.guardrails else 0.0 for result in attempted
        ),
        structured_output_rate=_average(
            1.0 if result.structured_output else 0.0 for result in attempted
        ),
        retry_feedback_rate=_average(
            1.0 if result.retry_feedback else 0.0 for result in attempted
        ),
        patch_diagnostics_rate=_average(
            1.0 if result.patch_diagnostics else 0.0 for result in attempted
        ),
        contextual_verifier_rate=_average(
            1.0 if result.contextual_verifier else 0.0 for result in attempted
        ),
        avg_process_quality_score=_average(
            result.process_quality_score for result in attempted
        ),
        process_quality_label_counts=_count_labels(
            result.process_quality_label for result in attempted
        ),
        process_quality_flag_counts=_count_labels(
            flag for result in attempted for flag in result.process_quality_flags
        ),
        process_risky_validated_tasks=sum(
            1
            for result in attempted
            if result.validation_passed and result.process_quality_label == "risky"
        ),
        live_provider_tasks=sum(
            1 for result in attempted if result.model_provider in LIVE_MODEL_PROVIDERS
        ),
        live_cost_budgeted_tasks=sum(
            1 for result in attempted if result.live_cost_budget_usd is not None
        ),
        live_cost_budget_overage_tasks=sum(
            1 for result in attempted if result.live_cost_budget_overage
        ),
        max_live_cost_budget_overage_usd=_max_optional_float(
            result.live_cost_budget_overage_usd for result in attempted
        ),
        model_provider=",".join(providers) if providers else None,
        response_count=_sum_optional(result.response_count for result in attempted),
        input_tokens=_sum_optional(result.input_tokens for result in attempted),
        output_tokens=_sum_optional(result.output_tokens for result in attempted),
        total_tokens=_sum_optional(result.total_tokens for result in attempted),
        estimated_cost_usd=_sum_optional_float(
            result.estimated_cost_usd for result in attempted
        ),
        ...
        max_attempted_task_tokens=_max_optional(
            result.total_tokens for result in attempted
        ),
        max_attempted_task_responses=_max_optional(
            result.response_count for result in attempted
        ),
        repeat_count=max((result.attempt_count for result in results), default=1),
        ...
        selected_virtual_file_count=_sum_optional(
            result.deepagents_virtual_file_count for result in selected
        ),
        ...
        partial_progress_tasks=sum(
            1
            for result in attempted
            if not result.validation_passed and result.progress_score >= 0.45
        ),
        avg_progress_score=_average(result.progress_score for result in attempted),
        selected_avg_progress_score=_average(
            result.progress_score for result in selected
        ),
        failure_class_counts=_count_labels(result.failure_class for result in attempted),
        selected_failure_class_counts=_count_labels(
            result.failure_class for result in selected
        ),
        harness_layer_counts=_count_labels(result.harness_layer for result in attempted),
        selected_harness_layer_counts=_count_labels(
            result.harness_layer for result in selected
        ),
    )


def _preflight_gate_blocked(result: ComplexBenchmarkResult, name: str) -> bool:
    return preflight_gate_blocked_from_gates(result.preflight_gates or [], name)
...
def _per_validated_result_cost(
    ...
    return _cost_per_success(
        costs=[result.estimated_cost_usd for result in attempted],
        success_count=sum(1 for result in attempted if result.validation_passed),
    )
...
def _per_validated_result_tokens(
    ...
    return _tokens_per_success(
        tokens=[result.total_tokens for result in attempted],
        success_count=sum(1 for result in attempted if result.validation_passed),
    )
...
def _per_validated_result_responses(
    ...
    return _tokens_per_success(
        tokens=[result.response_count for result in attempted],
        success_count=sum(1 for result in attempted if result.validation_passed),
    )
...
def _selected_virtual_files_per_validated_task(
    ...
    virtual_file_count = _sum_optional(
        result.deepagents_virtual_file_count for result in selected_results
    )
...
def _tokens_per_virtual_file(
    ...
    token_pairs = [
        (result.total_tokens, result.deepagents_virtual_file_count)
        for result in selected_results
        if result.total_tokens is not None
        and result.deepagents_virtual_file_count is not None
        and result.deepagents_virtual_file_count > 0
    ]
    if not token_pairs:
        return None
...
def _responses_per_virtual_file(
    ...
    response_pairs = [
        (result.response_count, result.deepagents_virtual_file_count)
        for result in selected_results
...
def _context_target_metrics(
    ...
    available = [
        result
        for result in selected_results
        if result.localized_target_paths and result.deepagents_virtual_file_paths
    ]
    if not available:
    ...
    for result in available:
        targets = set(result.localized_target_paths)
        context_paths = set(result.deepagents_virtual_file_paths)
